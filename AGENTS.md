...
- **The exit code decides whether it is retried.** Exit non-zero while the work is unfinished and the unit retries every 10s, indefinitely, which is how a device that boots without connectivity eventually provisions — nothing else re-triggers the hook while a healthy container keeps running. Exit 0 when there is nothing left to do, or when the remaining work should wait for the next app start instead. A hook that exits non-zero forever also leaves the unit `failed` and the system `degraded` between attempts, so reserve it for work that genuinely must complete.
