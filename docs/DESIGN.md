...
**Note on the install-time start**: postinst issues that restart itself, and `rules` passes `--no-start` to `dh_installsystemd` to suppress debhelper's own. debhelper's snippet calls `deb-systemd-invoke`, which runs `systemctl start` without `--no-block` and therefore waits for the unit's whole start transaction while dpkg holds its lock — for an app with a slow `ExecStartPre`, that stalls the package transaction behind it. `--no-block` returns as soon as the job is queued.
